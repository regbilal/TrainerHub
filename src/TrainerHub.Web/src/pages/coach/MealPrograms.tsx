...
import { UtensilsCrossed, Plus, Loader2, BookOpen, Users } from 'lucide-react';
import api from '../../lib/api';
import type { MealProgramListItem } from '../../types';

export default function CoachMealPrograms() {
  const { t } = useTranslation();
  const [programs, setPrograms] = useState<MealProgramListItem[]>([]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('coach.meals.title')}</h1>
        <Link
          to="/coach/meals/new"
      ...
      {programs.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">{t('coach.meals.noPrograms')}</h3>
          <p className="mt-1 text-sm text-gray-500">
            {t('coach.meals.noProgramsDesc')}
          </p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2">
          {programs.map((program) => (
            <Link
              key={program.id}
              to={`/coach/meals/${program.id}/edit`}
            ...
            >
              ...
              <div className="flex items-center gap-4 mt-4 text-xs text-gray-400">
                <span className="inline-flex items-center gap-1">
                  ...
                  {t('coach.meals.days', { count: program.dayCount })}
                </span>
                <span className="inline-flex items-center gap-1">
                  <Users className="h-3.5 w-3.5" />
                  {t('coach.meals.assigned', { count: program.assignmentCount })}
                </span>
              </div>
            </Link>
          ))}
